...
namespace Pathfinding {
	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
        ...
        public Vector2 velocity2D
        {
            get
            {
                Vector3 v = (this as IAstarAI).velocity;
                return new Vector2(v.x, v.y);
            }
        }

        public bool debugPathStart;
		...
		public void Teleport (Vector3 position, bool clearPath = true) {
			...
			simulatedPosition = previousPosition1 = previousPosition2 = position; //make velocity and prev. positions zero?
			if (updatePosition) tr.position = position;
			reachedEndOfPath = false;
			if (clearPath) SearchPath();
		}
        public void Teleport_preservePrevPosns(Vector3 position, bool clearPath = true)
        {
            if (clearPath) interpolator.SetPath(null);
            simulatedPosition = position; //make velocity and prev. positions zero?
            if (updatePosition) tr.position = position;
            reachedEndOfPath = false;
            if (clearPath) SearchPath();
        }
		...
		public virtual void SearchPath () {
            ...
            if(debugPathStart)
            {
                Debug.Log("Doing search path by: " + transform.parent.name);
            }
